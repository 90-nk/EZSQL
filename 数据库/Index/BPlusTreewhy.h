...
public:
    ...
    */
    ...
    //反序列化
    static BPlusTree deserialize(const std::string& filename, int degree) {
        std::ifstream file(filename, std::ios::binary | std::ios::in);
        if (!file) {
            throw std::runtime_error("无法打开文件进行读取");
        }

        BPlusTree tree(degree); // 使用有效的度数创建BPlusTree对象
        tree.root = deserializeNode(file);
        file.close();
        return tree;
    }
...
private:
    ...
    */
    ...
    //序列化
    void serializeNode(Node* node, std::ofstream& file) const {
        if (node == nullptr) {
            return;
        }
    ...
    //反序列化
    static Node* deserializeNode(std::ifstream& file) {
        if (file.peek() == std::ifstream::traits_type::eof()) {
            return nullptr;  // 检查文件结束
        }

        Node* node = new Node(); // 假设Node有默认构造函数
        int keyCount;
        ...
        if (!node->is_leaf) {
            for (int i = 0; i <= keyCount; ++i) {
                Node* child = deserializeNode(file);
...
int main (){
    BPlusTree<int, int> tree_save(3, {{1, 'A'}, {5, 'B'}, {7, 'C'}, {9, 'D'}, {10, 'E'}});
    tree_save.print();
    tree_save.serialize("test.dat");

    BPlusTree<int, int> tree_b = BPlusTree<int, int>::deserialize("test.dat",3);
    tree_b.print();
    return 0;
}
